...
export default function SocialCards({ cards }: SocialCardsProps) {
  ...
  return (
    <section className="flex flex-col items-center w-full py-4 lg:py-8 px-4 md:px-8 relative z-20">
      ...
      {/*
        Layout CSS the snippet depends on. The 21st.dev component only applies
        GSAP transforms (translate/rotate/scale) on top of a base layout — the
        cards must get their fixed size + centered absolute position from CSS,
        or they collapse to 0×0. Heights below match the breakpoints documented
        in getHeightMultiplier (22 / 26 / 28 / 34 / 38rem). Cards are 9:16 to
        match the rest of the site's reel posters and are centered via
        `inset:0; margin:auto` so GSAP's x/y offsets fan them out from center.
      */}
      <style>{`
        .fan-layout {
          height: 22rem;
        ...
        .fan-card {
          ...
          overflow: hidden;
          background: #0d0d0d;
          box-shadow: 0 24px 60px -18px rgba(13, 8, 4, 0.45), 0 4px 14px -6px rgba(13, 8, 4, 0.3);
        ...
        @media (min-width: 480px) {
          ...
          .fan-card { width: 9.28rem; height: 16.5rem; }
        }
        @media (min-width: 640px) {
          .fan-layout { height: 28rem; }
          .fan-card { width: 10.125rem; height: 18rem; border-radius: 1.15rem; }
        }
        @media (min-width: 768px) {
          .fan-layout { height: 34rem; }
          .fan-card { width: 11.8rem; height: 21rem; border-radius: 1.25rem; }
        }
        @media (min-width: 1024px) {
          .fan-layout { height: 38rem; }
          .fan-card { width: 13.5rem; height: 24rem; border-radius: 1.4rem; }
        }
        @media (prefers-reduced-motion: reduce) {
          .fan-card { will-change: auto; }
        }
      `}</style>
    </section>
  );
